...
2.CV回测，即将数据切分为N个训练窗口，选择N-k个为训练集，k个为测试集，进行 C(N-1, k-1)次循环计算。优点是充分考虑牛熊等多个市场状态，甚至是极端值，但是不是明确的历史路径解释，即重走一回不一定是这个收益。最终样本外决策可信度是比wf回测更高。但是仍旧只是基于历史数据单一路径进行的回测。AFML书里推荐设置K=2，来最大化训练集路径。
...
'''
第十八章 ：嫡特征
在本章中，我们将探讨如何确定价格序列中包含的信息量。机器学习算法可能会发现，当价格所含信息量很少时，势头交易更有利可图；而当价格所含信息量很大时，均值回归交易更有利可图。
